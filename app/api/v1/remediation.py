...
from app.cache_service import cache_service, cached, cache_invalidate
...
def get_remediation_status():
    ...
    try:
        ...
        db_metrics = db_service.get_performance_summary() if db_service.is_available() else {}
        ...
        logger.info(f"Remediation status requested - Engine: {status.get('enabled', False)}")
...
def test_remediation():
    ...
    try:
        ...
        test_metrics = {}
        metric_fields = ["cpu_percent", "memory_percent", "response_time_ms", "error_rate", "ml_anomaly_score", "ml_confidence"]
        ...
        for field in metric_fields:
            ...
            if value is not None:
                ...
                # Validate ranges
                if field in ["cpu_percent", "memory_percent"] and (value < 0 or value > 100):
                    return jsonify({
                        "status": "error",
                ...
                if field in ["ml_anomaly_score", "ml_confidence"] and (value < 0 or value > 1):
                    return jsonify({
                        "status": "error",
                        "error": f"{field} must be between 0 and 1",
                        "timestamp": datetime.now(timezone.utc).isoformat(),
        ...
        logger.info(f"Tested remediation with {len(triggered_actions)} actions triggered")
...
def get_remediation_metrics():
    ...
    try:
        ...
        logger.info(f"Retrieved remediation metrics - Success rate: {success_rate:.2f}%")
...
def get_remediation_health():
    ...
    try:
        ...
        db_health = db_service.health_check() if db_service.is_available() else {"status": "unavailable"}
        ...
        health_data = {
            ...
            "engine": {
                "status": "healthy" if engine_status.get("enabled", False) else "disabled",
