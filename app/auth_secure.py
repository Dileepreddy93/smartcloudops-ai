...
class SecureAPIKeyAuth:
        ...
        try:
            with self._lock:
                ...
                if self._is_ip_blocked(client_ip):
                    ...
                    return False, None, "IP address temporarily blocked due to repeated failures"
                ...
                if key_info.allowed_ips and client_ip not in key_info.allowed_ips:
                    self._log_auth_attempt(api_key, client_ip, user_agent, endpoint, required_permission, False)
                    return False, None, f"API key not authorized for IP address: {client_ip}"
                ...
                if required_permission not in key_info.permissions:
                    self._log_auth_attempt(api_key, client_ip, user_agent, endpoint, required_permission, False)
                    return False, None, f"Insufficient permissions: {required_permission} required"
    ...
    def _is_ip_blocked(self, ip_address: str) -> bool:
        ...
        self.blocked_ips[ip_address] = [
            timestamp for timestamp in self.blocked_ips[ip_address]
    ...
    def _log_auth_attempt(self, api_key: str, ip_address: str, user_agent: str,
                         endpoint: str, permission: str, success: bool):
