...
from app.utils.response import build_success_response, build_error_response
...
class SmartCloudOpsTask(Task):
    ...
    def on_success(self, retval, task_id, args, kwargs):
        ...
        cache_service.set(f"task_status:{task_id}", {
            'status': 'completed',
            'result': retval,
...
def train_ml_model(self, model_type: str = 'anomaly_detection', 
    ...
    try:
        ...
        cache_service.set(f"ml_training:{model_type}:{datetime.now(timezone.utc).strftime('%Y%m%d')}", 
                         training_result, ttl=86400)
...
@celery_app.task(base=SmartCloudOpsTask, bind=True, max_retries=3)
def process_metrics_data(self, metrics_data: List[Dict[str, Any]], 
    ...
    try:
        ...
        # Cache results
        cache_service.set(f"metrics_processing:{operation}:{datetime.now(timezone.utc).strftime('%Y%m%d_%H')}", 
                         results, ttl=3600)
...
def system_maintenance(self, maintenance_type: str = 'cleanup') -> Dict[str, Any]:
    ...
    try:
        ...
        cache_service.set(f"maintenance:{maintenance_type}:{datetime.now(timezone.utc).strftime('%Y%m%d')}", 
                         results, ttl=86400)
...
@celery_app.task(base=SmartCloudOpsTask, bind=True, max_retries=3)
def send_notifications(self, notification_type: str, 
                      recipients: List[str], 
                      message: str,
    ...
    try:
        ...
        cache_service.set(f"notification:{notification_type}:{datetime.now(timezone.utc).strftime('%Y%m%d_%H')}", 
                         results, ttl=3600)
...
def update_system_metrics(self) -> Dict[str, Any]:
    ...
    try:
        ...
        return build_success_response(
            data={'metrics_updated': True, 'anomaly_detected': anomaly_result.get('is_anomaly', False)},
