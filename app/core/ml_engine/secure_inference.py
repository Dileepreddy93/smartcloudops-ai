...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/tmp/ml_inference.log'),
...
class ModelConfig:
    ...
    def validate(self) -> bool:
        ...
        required_thresholds = {'cpu_threshold', 'memory_threshold', 'load_threshold', 'disk_threshold'}
        ...
        if missing_thresholds:
            raise ModelValidationError(f"Missing required thresholds: {missing_thresholds}")
        
        # Validate threshold values
        for key, value in self.thresholds.items():
            if not isinstance(value, (int, float)) or value < 0:
                raise ModelValidationError(f"Invalid threshold value for {key}: {value}")
        ...
        if missing_performance:
            raise ModelValidationError(f"Missing required performance metrics: {missing_performance}")
        ...
        for key, value in self.performance.items():
            if key == 'confusion_matrix':
                ...
                if isinstance(value, dict):
                    required_cm_keys = {'true_positives', 'false_positives', 'true_negatives', 'false_negatives'}
                    if not all(cm_key in value for cm_key in required_cm_keys):
                        raise ModelValidationError(f"Invalid confusion matrix structure: {value}")
                    ...
                    for cm_key, cm_value in value.items():
                        if not isinstance(cm_value, int) or cm_value < 0:
                            raise ModelValidationError(f"Invalid confusion matrix value for {cm_key}: {cm_value}")
                continue
                
            if not isinstance(value, (int, float)) or not (0 <= value <= 1):
                raise ModelValidationError(f"Invalid performance value for {key}: {value}")
        
        # Check model quality
        f1_score = self.performance.get('f1_score', 0)
        if f1_score < 0.7:
            logger.warning(f"Model F1-score ({f1_score:.3f}) is below recommended threshold (0.7)")
        
        # Validate data size
        if not isinstance(self.training_data_size, int) or self.training_data_size <= 0:
            raise ModelValidationError(f"Invalid training data size: {self.training_data_size}")
...
class SecureMLInferenceEngine:
    ...
    def __init__(self, model_path: Optional[str] = None, max_prediction_time: float = 5.0):
    ...
    def _load_and_validate_model(self, model_path: Optional[str] = None) -> ModelConfig:
        ...
        for path in model_paths:
            try:
                ...
                required_keys = {'model_type', 'training_timestamp', 'thresholds', 'performance', 'training_data_size'}
                if not all(key in raw_config for key in required_keys):
                    raise ModelValidationError(f"Missing required keys in model config: {required_keys - set(raw_config.keys())}")
                ...
                logger.info(f"📊 Model performance - F1: {model_config.performance['f1_score']:.3f}, "
        ...
        with self._lock:
            try:
                ...
                if self._model_config and collector_ok:
                    ...
                    logger.warning("Engine initialized in degraded mode (no data collector)")
                else:
                    self._health_status = HealthStatus.UNHEALTHY
                ...
                logger.info(f"ML Inference Engine initialized with status: {self._health_status.value}")
            ...
            try:
                ...
                if metric in ['cpu_usage', 'memory_usage', 'disk_usage']:
                    if not (0 <= float_value <= 100):
                        logger.warning(f"Metric {metric} out of range [0,100]: {float_value}")
                        float_value = max(0, min(100, float_value))
                elif metric == 'load_1m':
                    if not (0 <= float_value <= 100):  # Reasonable load limit
                        logger.warning(f"Load metric out of reasonable range: {float_value}")
                        float_value = max(0, min(100, float_value))
    ...
    def _calculate_dynamic_confidence(self, thresholds_exceeded: Dict[str, bool], 
        ...
        for metric, exceeded in thresholds_exceeded.items():
            if exceeded:
                ...
                actual_value = metrics.get(f"{metric_key}_usage" if metric_key != 'load' else 'load_1m', 0)
    ...
    def _collect_metrics_with_fallback(self) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            if collection_time > 10.0:  # 10 second timeout
                logger.warning(f"Metrics collection took too long: {collection_time:.2f}s")
    ...
    def predict_anomaly(self, metrics: Optional[Dict[str, Any]] = None, 
        ...
        try:
            with self._lock:
                ...
                logger.info(f"Prediction request {request_id} from user: {user_id or 'anonymous'}")
                ...
                # Apply thresholds
                thresholds_exceeded = {
                    'cpu_high': validated_metrics['cpu_usage'] > self._model_config.thresholds['cpu_threshold'],
                    'memory_high': validated_metrics['memory_usage'] > self._model_config.thresholds['memory_threshold'],
                    'load_high': validated_metrics['load_1m'] > self._model_config.thresholds['load_threshold'],
                    'disk_high': validated_metrics['disk_usage'] > self._model_config.thresholds['disk_threshold']
                }
                ...
                is_anomaly = exceeded_count >= 2 or (  # Require 2+ factors OR
                    exceeded_count >= 1 and (  # 1 factor if severe
                        validated_metrics['cpu_usage'] > self._model_config.thresholds['cpu_threshold'] * 1.5 or
                        validated_metrics['memory_usage'] > self._model_config.thresholds['memory_threshold'] * 1.5
                    )
                )
                ...
                logger.info(f"Prediction {request_id} complete: {status} "
            ...
            with self._lock:
                ...
                health = {
                    ...
                    'metrics': {
                        ...
                        'uptime_seconds': (current_time - self._last_health_check).total_seconds() if self._last_health_check else 0
                    }
                }
                ...
                if self._data_collector:
                    try:
                        ...
                        health['components']['prometheus_connection'] = test_metrics is not None
                    except:
...
if __name__ == "__main__":
    ...
    test_scenarios = [
        {
            'name': 'Normal System',
            'metrics': {'cpu_usage': 5.0, 'memory_usage': 30.0, 'load_1m': 0.1, 'disk_usage': 20.0}
        },
        {
            'name': 'High CPU',
            'metrics': {'cpu_usage': 85.0, 'memory_usage': 30.0, 'load_1m': 0.1, 'disk_usage': 20.0}
        },
        {
            'name': 'Multiple Issues',
            'metrics': {'cpu_usage': 85.0, 'memory_usage': 90.0, 'load_1m': 5.0, 'disk_usage': 95.0}
        },
        {
            'name': 'Invalid Input',
            'metrics': {'cpu_usage': 'invalid', 'memory_usage': None, 'load_1m': float('inf')}
        }
    ]
