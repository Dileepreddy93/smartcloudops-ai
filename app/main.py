...
Version: 3.2.0 Security Hardened with Centralized Configuration
"""
...
from auth_secure import (
    require_api_key, require_admin, require_ml_access, require_write_access,
    get_current_user, get_request_id, get_security_stats
)
from secure_api import (
    ValidationError, SecurityError, ErrorCode,
    StatusDTO, MLPredictionDTO, HealthCheckDTO,
    validate_request_data, sanitize_input, validate_ml_metrics,
...
# Import secure ML inference engine from proper package structure
ML_ENGINE_AVAILABLE = False
SecureMLInferenceEngine = None

try:
    from core.ml_engine import SecureMLInferenceEngine, MLInferenceEngine
    if SecureMLInferenceEngine is not None:
        ML_ENGINE_AVAILABLE = True
        logging.info("✅ SecureMLInferenceEngine imported successfully")
    elif MLInferenceEngine is not None:
        SecureMLInferenceEngine = MLInferenceEngine
        ML_ENGINE_AVAILABLE = True
        logging.info("✅ Production MLInferenceEngine imported as fallback")
    else:
        logging.warning("⚠️ No ML engines available from core package")
except ImportError as e:
    logging.warning(f"⚠️ Failed to import from core.ml_engine: {e}")
    
    # Fallback for development/testing
    try:
        import sys
        import os
        # Add scripts directory as fallback
        ...
        ML_ENGINE_AVAILABLE = True
        logging.info("✅ Fallback: SecureMLInferenceEngine imported from scripts")
    except ImportError:
        try:
            from production_inference import MLInferenceEngine
            SecureMLInferenceEngine = MLInferenceEngine
            ML_ENGINE_AVAILABLE = True
            logging.info("✅ Fallback: ProductionMLInferenceEngine imported from scripts")
        except ImportError:
            logging.error("❌ No ML inference engines available - ML predictions will be disabled")

if not ML_ENGINE_AVAILABLE:
    logging.warning("⚠️ Application will run without ML prediction capabilities")
    SecureMLInferenceEngine = None
...
def handle_security_error(e):
    ...
    logger.error(f"SECURITY VIOLATION: {str(e)} - IP: {request.remote_addr} - Request ID: {getattr(g, 'request_id', 'unknown')}")
    return jsonify(build_error_response(
        ErrorCode.VALIDATION_ERROR,
...
def chat():
    ...
    try:
        ...
        context = sanitize_input(data.get('context', ''), max_length=500) if data.get('context') else None
...
def ml_predict():
    ...
    try:
        ...
        model_version = sanitize_input(data.get('model_version', 'latest'), max_length=50) if data.get('model_version') else 'latest'
        ...
        try:
            ...
            logger.info(f"ML prediction completed - Anomaly: {prediction_data.anomaly_detected}, "
                       f"Confidence: {prediction_data.confidence_score}, User: {user.get('user_id', 'unknown')}")
            
            return jsonify(build_success_response(
                prediction_data.to_dict(),
                request_id=get_request_id()
            )), 200
