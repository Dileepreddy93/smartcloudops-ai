...
def get_status():
    ...
    try:
        ...
        logger.info(f"Status requested - Database: {db_status}, Cache: {cache_status}, ML: {ml_status}")
...
def system_metrics():
    ...
    try:
        ...
        db_metrics = db_service.get_performance_summary() if db_service.is_available() else {"status": "unavailable"}
        ...
        metrics_data = {
            ...
            "database": db_metrics,
            "services_status": {
                "database": "operational" if db_service.is_available() else "unavailable",
...
def main():
    ...
    logger.info(f"🔧 Database: {'Available' if db_service.is_available() else 'Unavailable'}")
    ...
    logger.info(f"🔧 Authentication: {'Available' if AUTH_AVAILABLE else 'Unavailable'}")
    logger.info("=" * 60)
