...
try:
    ...
    # Define metrics
    REQUEST_COUNT = Counter('smartcloudops_requests_total', 'Total requests', ['method', 'endpoint'])
    REQUEST_DURATION = Histogram('smartcloudops_request_duration_seconds', 'Request duration')
    ML_PREDICTIONS = Counter('smartcloudops_ml_predictions_total', 'Total ML predictions', ['status'])
    AI_REQUESTS = Counter('smartcloudops_ai_requests_total', 'Total AI requests', ['provider'])
    ...
    try:
        if ml_engine_preference == 'real_data' and RealDataInferenceEngine:
            ...
            try:
                ...
                logger.warning("Secure ML Engine not available, falling back to real_data")
                ml_engine_instance = RealDataInferenceEngine()
                ML_ENGINE_TYPE = "real_data"
...
def status():
    ...
    return jsonify({
        ...
        'auth': {
            ...
            'user': request.auth_info.get('name', 'unknown') if hasattr(request, 'auth_info') else None
        }
    })
...
def chat():
    ...
    try:
        ...
        user_name = request.auth_info.get('name', 'unknown') if hasattr(request, 'auth_info') else 'anonymous'
...
def ml_predict():
    ...
    try:
        ...
        user_name = request.auth_info.get('name', 'anonymous') if hasattr(request, 'auth_info') else 'anonymous'
        ...
        logger.info(f"ML prediction request from {user_name}: "
