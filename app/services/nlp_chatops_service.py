...
class NLPEnhancedChatOps:
                    ...
                    for i, match in enumerate(matches):
                        if isinstance(match, tuple):
                            for j, group in enumerate(match):
                                if group:
                                    entity_name = intent_config["entities"][j] if j < len(intent_config["entities"]) else f"entity_{j}"
    ...
    def _generate_action_plan(self, intent: str, entities: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if intent == "deploy":
            ...
            count = entities.get("count", entities.get("numbers", [1])[0] if entities.get("numbers") else 1)
            action_plan["aws_operations"] = [
    ...
    def _log_command(self, original_input: str, intent: str, entities: Dict[str, Any], action_plan: Dict[str, Any]):
