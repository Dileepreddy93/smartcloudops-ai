...
def monitor_workflows():
    ...
    while True:
        ...
        print(
            f"🔍 GitHub Workflows Monitor - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        print("=" * 60)
        ...
        for run in runs:
            ...
            print(f"   ID: {run['id']}")
            print(
                f"   Status: {format_status(run['status'], run.get('conclusion', ''))}"
            )
            ...
            if run["status"] == "completed":
                ...
                if jobs:
                    print("   Jobs:")
                    for job in jobs:
                        job_status = format_status(
                            job["status"], job.get("conclusion", "")
                        )
