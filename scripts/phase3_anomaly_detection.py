...
class SmartCloudOpsAnomalyDetector:
    ...
    def prepare_features(self, df):
        ...
        for col in numeric_cols:
            if col not in ['timestamp', 'hour', 'day_of_week', 'is_weekend', 'is_anomaly']:
    ...
    def create_visualizations(self, df, results):
        ...
        try:
            ...
            fig.suptitle('SmartCloudOps AI - Anomaly Detection Analysis', fontsize=16, fontweight='bold')
            
            # Plot 1: Time series with anomalies
            ax1 = axes[0, 0]
            df_plot = df.tail(1000)  # Plot last 1000 points for visibility
            
            ax1.plot(df_plot['timestamp'], df_plot['cpu_usage'], 'b-', alpha=0.7, label='CPU Usage')
            ax1.plot(df_plot['timestamp'], df_plot['memory_usage'], 'g-', alpha=0.7, label='Memory Usage')
            
            # Highlight anomalies
            if 'is_anomaly' in df_plot.columns:
                anomalies = df_plot[df_plot['is_anomaly'] == 1]
                ax1.scatter(anomalies['timestamp'], anomalies['cpu_usage'], 
            ...
            if 'anomaly_scores' in results:
                ax2.hist(results['anomaly_scores'], bins=50, alpha=0.7, color='skyblue', edgecolor='black')
