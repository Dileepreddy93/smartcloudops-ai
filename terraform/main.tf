# SmartCloudOps AI Infrastructure
# Terraform configuration for AI-powered cloud operations platform
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        {
          name  = "DATABASE_URL"
          value = "mysql://${var.db_username}:${var.db_password}@${aws_db_instance.main.endpoint}:3306/smartcloudops"
        },
        {
          name  = "S3_ML_MODELS_BUCKET"
...
resource "aws_ecs_service" "main" {
  ...
  load_balancer {
    ...
    container_name   = "smartcloudops-ai"
    container_port   = 8000
  }
...
# Lambda function for ML model training/inference (optional)
# Note: Create ml_processor.zip file before applying this resource
resource "aws_lambda_function" "ml_processor" {
  count = var.enable_lambda ? 1 : 0

  filename         = "${path.module}/lambda/ml_processor.zip"
  function_name    = "${var.project_name}-ml-processor"
  role             = aws_iam_role.lambda[0].arn
  handler          = "lambda_function.lambda_handler"
  source_code_hash = fileexists("${path.module}/lambda/ml_processor.zip") ? filebase64sha256("${path.module}/lambda/ml_processor.zip") : null
  runtime          = "python3.9"
  timeout          = var.lambda_timeout

  environment {
    variables = {
      S3_ML_MODELS_BUCKET = aws_s3_bucket.ml_models.id
