...
class TestRemediationEngine:
    ...
    def test_process_metrics_with_ml_prediction(self):
        ...
        with patch.object(engine, '_restart_service', return_value=True), \
             patch.object(engine, '_send_alert', return_value=True):
...
class TestIntegrationService:
    ...
    def test_get_recent_metrics(self):
        ...
        for i in range(5):
            metrics = {"cpu_percent": i * 10, "timestamp": datetime.utcnow().isoformat()}
...
class TestRemediationAPI:
    ...
    def test_set_manual_override(self, client):
        ...
        response = client.post('/api/v1/remediation/override', 
                             json={"enabled": True})
